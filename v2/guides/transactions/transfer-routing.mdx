...
<Note>`force_internal` and `force_external` are mutually exclusive: setting both to `true` in the same request is invalid and returns an HTTP 400 error (error code `2006`). To force a specific routing, set only one to `true` and leave the other as `false` (or omit it).</Note>
