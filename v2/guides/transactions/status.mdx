...
## Statuses and webhook events

Status and sub-status changes are surfaced through the `wallets.transaction.*` webhook events. `Confirming`, including sub-statuses such as `PendingBlockConfirmations`, is a nonterminal, in-progress status reported through `wallets.transaction.updated` as confirmations accumulate. `Completed` is a terminal status: it is reported through `wallets.transaction.updated` like any other transition and additionally produces a dedicated `wallets.transaction.succeeded` event. This gives you an unambiguous positive completion signal without inspecting `status`. For the full event catalog and subscription guidance, see [Transaction event subscriptions](/v2/guides/webhooks-callbacks/transaction-event-subscriptions).
