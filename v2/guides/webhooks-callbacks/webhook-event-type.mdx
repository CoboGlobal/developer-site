...
<table class="table-three-cols">
  ...
  <tbody>
    <tr>
      ...
      <td>A transaction record has been created. For an inbound transaction such as a deposit, this follows on-chain detection. For an outbound transaction such as a withdrawal, this follows Cobo creating the record from the request. This event does not indicate that the transaction was successful.</td>
    </tr>
    <tr>
      <td><code>wallets.transaction.updated</code></td>
      <td><code>Transaction</code></td>
      <td>There are changes to a transaction's status or confirmation numbers.</td>
    </tr>
    <tr>
...
For the complete Transaction payload field reference and worked examples, see [Transaction payload reference](/v2/guides/webhooks-callbacks/wallets-transaction-payload).

### Event overlap for a single transaction

| Event | Relationship to other events |
| --- | --- |
| `created` | Attempted, in addition to `updated`, when the transaction record is first created. |
| `updated` | Attempted for every processed `status` or `sub_status` transition across the transaction lifecycle, including the final transition. |
| `succeeded` / `failed` | Attempted, in addition to `updated`, as a dedicated terminal notification when the transaction reaches `Completed` or a failure or rejection outcome. |

If you subscribe only to `updated`, you must inspect the payload's `status` and `sub_status` to detect completion or failure yourself. Subscribing to `succeeded` and `failed` as well gives you a dedicated, unambiguous terminal signal without payload inspection. See [Set up a callback or webhook endpoint](/v2/guides/webhooks-callbacks/set-up-endpoint) for ordering and deduplication guidance.
...
`wallets.mpc.tss_request` is the only supported `wallets.mpc` event family.
...
For the complete TSSRequest payload field reference, producer scope, and worked examples, see [TSS request payload reference](/v2/guides/webhooks-callbacks/wallets-mpc-tss-request-payload).
...
For a complete introduction of webhook event data and its structure, refer to the [data](/v2/api-references/developers--webhooks/retrieve-event-information#response-data) property in the response of the Retrieve event information operation or the [data.data](/v2/api-references/developers--webhooks/list-all-webhook-events#response-data-data) property in the response of the List all webhook event operation. For narrative field references and examples, see [Transaction payload reference](/v2/guides/webhooks-callbacks/wallets-transaction-payload) and [TSS request payload reference](/v2/guides/webhooks-callbacks/wallets-mpc-tss-request-payload).
