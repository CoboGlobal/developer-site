...
`fee_station.transaction.*` events deliver the same `Transaction` payload schema as `wallets.transaction.*` events. The webhook code uses the same transaction detail serializer for both event families; the Fee Station branch only chooses the `fee_station.` event-type prefix and removes wallet-scope metadata fields from `wallet_scopes_info` (`custody/waas2/webhooks/managers/wallet_event_by_source.py:34-113`, `custody/waas2/webhooks/managers/wallet_event.py:42-44`).

The `type` field does not distinguish Fee Station transactions. Internal Fee Station query types are mapped to the public `Deposit` or `Withdrawal` values before the payload is returned (`custody/waas2/transactions/mongo/manager/read/waas_transaction.py:712-727`, `custody/waas2/transactions/dev/enums/transaction.py:104-143`).
...
<Note>Use `cobo_category` to identify Fee-Station-specific transaction categories. The Fee Station values are defined by `TransactionCoboCategory` (`custody/waas2/transactions/mongo/enums/transaction_fields.py:45-63`) and are documented in [Cobo-defined categories](/v2/guides/transactions/manage-transactions#cobo-defined-categories).</Note>
