...
## Endpoint scope and reconciliation

Each webhook endpoint is configured once for your organization, with the event types you select at registration and, optionally, a wallet scope limiting which wallets' events it receives. Register or update this scope on Cobo Portal. See [Register a webhook endpoint](https://manuals.cobo.com/en/portal/developer-console/webhooks-create).

Because retries can redeliver the same event, and because both `wallets.transaction.updated` and a terminal event (`succeeded` or `failed`) can describe the same transaction, treat webhook events as notifications rather than the system of record. Deduplicate deliveries using the event's `event_id`. To reconcile the current, authoritative transaction state, call [Get transaction information](/v2/api-references/transactions/get-transaction-information) or [List all transactions](/v2/api-references/transactions/list-all-transactions) using `transaction_id`, or `cobo_id`, `request_id`, or `transaction_hash` when you have one of those identifiers, instead of trusting the last webhook payload alone.

See [Transaction event subscriptions](/v2/guides/webhooks-callbacks/transaction-event-subscriptions) for scenario-level processing guidance.
