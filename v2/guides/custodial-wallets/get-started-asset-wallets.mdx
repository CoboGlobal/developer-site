...
Before you deposit tokens, make sure the chain is activated for your organization and the token is listed and enabled for deposits. A token accepts deposits only when its `can_deposit` property is `true`. Confirm this property before you send funds, because deposits made to a chain or token that is not yet enabled may not be detected or credited automatically.

To check deposit eligibility, query the token configuration before depositing. If you have already sent a deposit to a token that was not enabled at the time, do not send further funds to the same address; instead, confirm the current status of the transfer with the [List all transactions](/v2/api-references/transactions/list-all-transactions) operation.

## Memo and tag requirements

Some chains require a memo or tag in addition to the deposit address so that the deposit can be routed to the correct account. When a token requires a memo or tag, its `require_memo` property is `true`. Always check this property before you deposit, and include the exact memo or tag with every deposit to that token.

A deposit sent without the required memo or tag, or with an incorrect memo or tag, may not be credited automatically and can require manual processing before the funds appear in your wallet. To confirm whether such a deposit has been received, use the [List all transactions](/v2/api-references/transactions/list-all-transactions) operation.
...
Some chains support only specific transaction formats or versions for deposits. A deposit submitted in an unsupported transaction format may not be detected or credited. When you integrate a new chain, verify that your deposits use a supported transaction format before sending funds at scale.
