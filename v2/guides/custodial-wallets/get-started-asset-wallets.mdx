...
If you expect a deposit from this type of transfer and it does not appear, first verify that the transaction was completed on-chain using the relevant blockchain explorer, then use the [List all transactions](/v2/api-references/transactions/list-all-transactions) operation to check whether it has been recorded by Cobo. If no record is found, contact Cobo support for assistance.
