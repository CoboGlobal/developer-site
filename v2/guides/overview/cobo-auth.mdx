...
- Biz-Api-Nonce: The current Unix timestamp in milliseconds. This value is used as the request nonce.
...
In WaaS 2.0 authentication, Nonce refers to the value of the `Biz-Api-Nonce` request header. This value must be the current Unix timestamp in milliseconds.
...
1. First, concatenate a string based on your request as follows:
    ...
    str_to_sign = `{METHOD}|{PATH}|{NONCE}|{PARAMS}|{BODY}`

    | **Field** | **Description**                                              | **Example**                                                  |
    | --------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
    | METHOD    | The HTTP method.                                             | `GET`                                                          |
    | PATH      | The API endpoint.                                            | `/v2/transactions/transfer`                                    |
    | NONCE     | The current Unix timestamp in milliseconds. This value must exactly match the value of the `Biz-Api-Nonce` request header. | `1718587017026`                                                |
