...
This version was released on September 16, 2026.

### Compatibility changes

- The `address_owner`, `spender`, and `approver` response fields in [List approval details](/v2/api-references/transactions/list-approval-details) now allow `user_details[].result` to be `Pending`, in addition to `Approved` and `Rejected`.

## v1.43.0

This version was released on September 2, 2026.

This release contains no API or SDK changes.
...
- Added the following Reconciliation operations for selected-customer stablecoin reconciliation:
  - [Get reconciliation ledger](/v2/api-references/reconciliation/get-reconciliation-ledger)
  - [List reconciliation daily statements](/v2/api-references/reconciliation/list-reconciliation-daily-statements)
- Added [List account balances](/v2/api-references/wallets/list-account-balances) for retrieving up to 50 token/address balances from the same block state on an account-based chain. Each returned balance includes the block number.
- Added [Get transaction receipt](/v2/api-references/transactions/get-transaction-receipt) for retrieving EVM transaction receipt data, including execution status, block data, gas usage, and raw logs.
- Enhanced [Transfer token](/v2/api-references/transactions/transfer-token) and [Call smart contract](/v2/api-references/transactions/call-smart-contract) to support `fee_level` and `transaction_process_type=SignOnly`. Sign-only transactions can be broadcast with [Broadcast signed transactions](/v2/api-references/transactions/broadcast-signed-transactions).
- Added `ERC20Fund` as a tokenization token standard in [Issue token](/v2/api-references/tokenization/issue-token), with ERC-4626 fund/vault parameters, NAV controls, deposit/redemption limits, rate controls, and role permissions.

### Compatibility changes

- Transaction responses now include `transaction_process_type`, and Filecoin transaction fee details now include `gas_used`. Affected operations:
...
- Fee Station Auto-Fuel support now includes eligible Web3 Wallet transactions in addition to MPC Wallet transactions.
- Removed the `List Delegates` operation (`POST /wallets/{wallet_id}/smart_contracts/delegates`) for Smart Contract Wallets.
- Removed the `SmartContract` wallet type and `Safe{Wallet}` subtype from wallet enums and schemas. Affected operations:
  - [Create wallet](/v2/api-references/wallets/create-wallet)
  - [List all wallets](/v2/api-references/wallets/list-all-wallets)
  - [Get wallet information](/v2/api-references/wallets/get-wallet-information)
  - [Update wallet](/v2/api-references/wallets/update-wallet)
- Removed Smart Contract Wallet source/delegate schemas from [Transfer token](/v2/api-references/transactions/transfer-token) and [Call smart contract](/v2/api-references/transactions/call-smart-contract). The contract-call operation is now scoped to MPC transactions on EVM-like chains.
- Removed `Safe{Wallet}` transaction schema support, including `TransactionSourceType=Safe{Wallet}`, `TransactionType=ExternalSafeTx`, `TransactionCoboCategory=SafeTxMessage`, Safe transaction source schemas, and EIP-712 `safe_tx_extra_data`.
- The following operations are now applicable to MPC Wallets only:
  - [List token balances by address](/v2/api-references/wallets/list-token-balances-by-address)
  - [Cancel transaction](/v2/api-references/transactions/cancel-transaction)
...
This version was released on August 5, 2026.

This release contains no API or SDK changes.

## v1.40.0

This version was released on July 22, 2026.

This release contains no API or SDK changes.

## v1.39.0

This version was released on July 9, 2026.

### New features

- Added the `token_addresses` query parameter to [List enabled tokens](/v2/api-references/wallets/list-enabled-tokens) for filtering by token contract address (supports up to 500 comma-separated addresses, case-insensitive). Because the same contract address may exist on multiple chains, combine with `chain_ids` to narrow results. Native tokens have no contract address and are excluded.
- Added the `action` query parameter to [List transaction templates](/v2/api-references/transactions/list-transaction-templates) for specifying the approval action type (`Transfer`, `Drop`, or `SpeedUp`). Defaults to `Transfer` when omitted.
- Enhanced [Get transaction information](/v2/api-references/transactions/get-transaction-information) and [List all transactions](/v2/api-references/transactions/list-all-transactions) to include `FeePayer` in `transaction.extra` when `extra_type=FeePayer`, providing the designated Solana fee payer address.
- Added the `action` field to `user_details` in approval responses. Affected operations:
  - [Get transaction approval details](/v2/api-references/transactions/get-transaction-approval-details)
  - [List transaction approval details](/v2/api-references/transactions/list-transaction-approval-details)

### Compatibility changes

- Deprecated `Raw_Message_Signature` in the [Sign message](/v2/api-references/transactions/sign-message) operation due to security concerns. It is no longer allowed and must not be used. It may be removed in a future version.
- Clarified wallet name constraints in the [Create wallet](/v2/api-references/wallets/create-wallet) operation: the `name` field is required and cannot be empty; special characters such as `@` are not accepted and will return `400 Bad Request`.
- Clarified that fee estimates from [Estimate transaction fee](/v2/api-references/transactions/estimate-transaction-fee) are point-in-time and short-lived. Re-estimate immediately before submitting a withdrawal to avoid rejection due to insufficient fees.
- Clarified async behavior of [Create auto sweep task](/v2/api-references/autosweep/create-auto-sweep-task): returned tasks have `status` set to `Submitted` with an empty `transaction_ids` array; poll [Get auto sweep task details](/v2/api-references/autosweep/get-auto-sweep-task-details) until `status` becomes `TransactionCreated`. Sweeps require native chain coin for gas from the deposit address, Fee Station, or Auto-Fuel.
- Clarified that `transaction_hash` in [Get transaction information](/v2/api-references/transactions/get-transaction-information) and [List all transactions](/v2/api-references/transactions/list-all-transactions) responses may be `null` or absent before on-chain broadcast.
- Clarified Solana transaction fee fields: `fee_used` covers on-chain network fees only and excludes ATA activation rent. `rent_amount` in fee estimate responses is reserved for future ATA activation rent and is currently not populated.

## v1.38.0

This version was released on June 25, 2026.

### Compatibility changes

- Added the `result_token` field to `user_details` in the response of the [Get transaction approval details](/v2/api-references/transactions/get-transaction-approval-details) operation. When verifying approval signatures, the full signing content is constructed as `{message}||{result_token}`.
- Clarified that the `PendingAuthorization` transaction status covers both manual approval steps and internal automated risk-control, KYT, and compliance screening checks. A transaction in this status does not necessarily require human action. See [Transaction statuses and sub-statuses](/v2/guides/transactions/status).
...
- Added the following new webhook event types:
  - `wallets.updated`
  - `mpc_vaults.updated`
  - `mpc_vaults.deleted`
  - `organizations.updated`
- Added two new webhook event data types, `Organization` and `FiatTransaction`, to the webhook event payload. Affected operations:
...
- Added `UsePortalPreference` as a new option to the `AutoFuelType` enum for [Fee Station](https://manuals.cobo.com/en/portal/billing-center/introduction) transactions. This option allows you to use payment preferences set on Cobo Portal instead of specifying the mode of transaction fee payment in each API call.
...
- Added the [Estimate fee for Fee Station transaction](/v2/api-references/feestation/estimate-fee-for-fee-station-transaction) operation to estimate the gas fee for top-up transactions sent by [Fee Station](https://manuals.cobo.com/en/portal/billing-center/introduction).
...
- Added the `auto_fuel` property to the request payloads of the following operations to enable the use of [Fee Station](https://manuals.cobo.com/en/portal/billing-center/introduction) for automatic transaction fee payment:
  - [Transfer token](/v2/api-references/transactions/transfer-token)
  - [Call smart contract](/v2/api-references/transactions/call-smart-contract)
  - [Speed up transaction](/v2/api-references/transactions/speed-up-transaction)
  - [Drop transaction](/v2/api-references/transactions/drop-transaction)
...
* Added the List Delegates operation for Smart Contract Wallets. (This operation has since been retired.)  
...
- Added support for [Exchange Wallets](https://manuals.cobo.com/en/portal/exchange-wallets/introduction) and Smart Contract Wallets. Added the following operations for Exchange Wallets:
