...
| 1006, 2006 | One or more parameters are in an invalid format or contain unsupported values. | The `error_message` field names the parameter that failed validation. Check the API reference for the operation you are calling to confirm the expected type, format, and allowed values for that parameter, then correct the value and retry. If `error_message` names `account_output.amount`, see [Address destination amount](/v2/guides/transactions/sources-and-destinations#address-destination-amount) for amount format, token decimal precision, positivity, and applicable Custodial Wallet `dust_threshold` requirements. |
...
| 30013 | The fee token balance is insufficient to cover the transaction fee. | Ensure the wallet or Billing Center has sufficient balance in the fee token (for example, ETH on EVM chains) to cover the transaction fee. |
...
| 30036 | The Billing Center balance is insufficient. Transactions cannot proceed until it is topped up. | In Cobo Portal, go to **Billing Center** and click **Deposit**. Choose **Stablecoin** or **USD** to top up Credits, or **Gas token** to deposit the native gas token required by the target network. Retry after the balance is updated. |
...
| 30038 | The Billing Center balance is insufficient to cover the transaction fee. | In Cobo Portal, go to **Billing Center** and click **Deposit**. Deposit Credits, or choose **Gas token** to deposit the exact native gas token required by the network for this transaction. Retry after the balance is sufficient. |
| 30039 | Cobo is temporarily unable to sponsor the gas fee for this transaction. | This error is caused by a temporary condition on Cobo's side. Retry after a short delay. |
