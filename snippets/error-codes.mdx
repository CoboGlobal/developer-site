...
| 1006, 2006 | One or more parameters are in an invalid format or contain unsupported values. For transfer operations, this includes amount format or decimal-precision errors, for example, when `account_output.amount invalid` appears in `error_message` because the submitted amount has more decimal places than the token supports. | The `error_message` field names the parameter that failed validation. If the parameter is an amount field (for example, `account_output.amount invalid`), call the [Get token information](/v2/api-references/wallets/get-token-information) operation to retrieve the token's `decimal` precision and ensure the submitted amount does not exceed it. For all other parameters, check the API reference for the operation you are calling to confirm the expected type, format, and allowed values for that parameter, then correct the value and retry. |
...
| 30012, 12007 | The wallet's available balance or fee balance is insufficient to complete the operation. The `12007` code can also indicate an insufficient fee-token or Fee Station balance when `error_message` describes a fee-related shortfall. | Check `error_message` to identify which balance is insufficient. If it refers to the wallet's transfer balance, the available balance equals the total balance minus pending and reserved amounts; call the [Get wallet balance](/v2/api-references/wallets/get-wallet-balance) operation to verify, top up the wallet, and retry. If `error_message` for error `12007` indicates an insufficient fee-token or Fee Station balance, in Cobo Portal, go to **Fee Station**, top up the Gas Token balance for the network being used or the USD / USD Stablecoins balance as required by your payment preference, and retry. |
