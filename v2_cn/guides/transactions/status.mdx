...
<table className="table-three-cols">
  ...
  <tr>
    ...
    <td>交易正在等待签名人在 Cobo Guard 签名。</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>原交易被新交易替换（通过 Replace-By-Fee（RBF）机制实现）。</td>
  </tr>
  <tr>
...
状态和子状态的变化通过 `wallets.transaction.*` Webhook 事件呈现。`Confirming` 及其子状态（包括 `PendingBlockConfirmations`）是非终态的进行中状态，随着确认数累积通过 `wallets.transaction.updated` 报告。`Completed` 是终态：与其他状态转换一样，它通过 `wallets.transaction.updated` 报告，同时还会产生专用的 `wallets.transaction.succeeded` 事件。这样，您无需检查 `status` 即可获得明确的成功完成信号。有关完整事件列表和订阅指南，请参阅 [Transaction 事件订阅](/v2_cn/guides/webhooks-callbacks/transaction-event-subscriptions)。
