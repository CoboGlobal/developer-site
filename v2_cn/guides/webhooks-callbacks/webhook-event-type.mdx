...
<Note>
这些事件由 Cobo 发送，描述的是 Cobo 侧所见的 TSS 请求状态。它们与您自己部署的 TSS Node 直接推送到您服务器的事件通知不是同一套机制——后者使用 `request.*` 事件名，由节点的 RSA 密钥签名，并在节点配置文件中配置。在此处订阅不会收到后者。详见 [TSS Node 事件通知](/v2_cn/guides/mpc-wallets/server-co-signer/tss-node-event-notifications)。
</Note>
