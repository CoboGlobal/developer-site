...
<table class="table-three-cols">
  ...
  <tbody>
    <tr>
      ...
      <td>已创建交易记录。对于充币等入账交易，会在链上检测到交易后创建记录；对于提币等出账交易，会在 Cobo 根据请求创建记录后触发。此事件不代表交易已成功。</td>
    </tr>
    <tr>
...
有关完整的 Transaction 负载字段参考和示例，请参阅 [Transaction 负载参考](/v2_cn/guides/webhooks-callbacks/wallets-transaction-payload)。

### 单笔交易的事件重叠

| 事件 | 与其他事件的关系 |
| --- | --- |
| `created` | 创建交易记录时，除 `updated` 外还会尝试触发此事件。 |
| `updated` | 交易生命周期内每次处理 `status` 或 `sub_status` 转换时都会尝试触发，包括最终转换。 |
| `succeeded` / `failed` | 交易达到 `Completed` 或失败、拒绝结果时，除 `updated` 外还会尝试触发此专用终态通知。 |

如果仅订阅 `updated`，您必须检查负载中的 `status` 和 `sub_status`，自行识别完成或失败。额外订阅 `succeeded` 和 `failed` 可获得明确且无需检查负载的专用终态信号。有关顺序和去重指南，请参阅 [设置 Callback 或 Webhook Endpoint](/v2_cn/guides/webhooks-callbacks/set-up-endpoint)。
...
`wallets.mpc.tss_request` 是目前唯一支持的 `wallets.mpc` 事件类型系列。
...
有关完整的 TSSRequest 负载字段参考、产生范围和示例，请参阅 [TSS request 负载参考](/v2_cn/guides/webhooks-callbacks/wallets-mpc-tss-request-payload)。
...
## Webhook 事件数据结构
有关 Webhook 事件数据及其结构的完整介绍，请参阅 Retrieve event information 操作响应中的 [data](/v2/api-references/developers--webhooks/retrieve-event-information#response-data) 属性，或 List all webhook events 操作响应中的 [data.data](/v2/api-references/developers--webhooks/list-all-webhook-events#response-data-data) 属性。有关字段说明和示例，请参阅 [Transaction 负载参考](/v2_cn/guides/webhooks-callbacks/wallets-transaction-payload)和 [TSS request 负载参考](/v2_cn/guides/webhooks-callbacks/wallets-mpc-tss-request-payload)。
