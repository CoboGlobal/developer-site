...
您可以使用 [List all webhook events](/v2/api-references/developers--webhooks/list-all-webhook-events) 查询指定 Endpoint 最多 30 天的 Webhook 事件历史。请使用此查询窗口检测缺失事件，而不要仅依赖自己的投递日志。

对于特定事件，请使用 [Retrieve event information](/v2/api-references/developers--webhooks/retrieve-event-information) 检查其当前状态和负载，并使用 [List webhook event logs](/v2/api-references/developers--webhooks/list-webhook-event-logs) 查看每次投递尝试。每次重试都会产生一条单独的日志记录。

Endpoint 恢复可访问后，请调用 [Retry event](/v2/api-references/developers--webhooks/retry-event) 恢复停留在 `Retrying` 或 `Failed` 的事件。此操作相当于在 Cobo Portal 上点击 **Retry**。

30 天窗口用于查询投递历史，并不保证交易当前状态与最后投递的事件一致。请始终通过 [Get transaction information](/v2/api-references/transactions/get-transaction-information) 或 [List all transactions](/v2/api-references/transactions/list-all-transactions) 核对权威状态，而不要仅信任最近的 Webhook 负载。
