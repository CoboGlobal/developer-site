...
交易类事件的交易内容位于事件的 `data` 属性中。对手方地址与金额在其中的位置取决于 `data.destination.destination_type`，因此请先读取该字段并据此分支，不要假定固定路径。

充值（`destination_type` 为 `DepositToAddress`）将两者直接放在 `destination` 下：
...
提币到地址（`destination_type` 为 `Address`）则多嵌套一层，放在 `account_output` 下：
...
<Warning>
  两种结构在交易顶层都没有 `address` 或 `amount`。只按其中一种结构编写的处理逻辑会静默漏掉另一种；开发阶段收到的第一笔交易往往是充值，因此这一点很容易被忽略。
</Warning>

以上两种是最先遇到的结构。`destination_type` 还有其他取值，例如 `CustodialWallet`、`ExchangeWallet`、各链的合约调用类型与消息签名类型，各自结构不同。建议只处理您的集成预期的类型，其余记录日志，而不要假定其结构。
