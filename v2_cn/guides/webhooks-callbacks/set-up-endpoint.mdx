...
1. 获取请求包体的原始数据和时间戳。
    ...
    <Note>
    每个请求头都会以两种拼写发送：`BIZ_TIMESTAMP` 与 `BIZ-TIMESTAMP`（签名头同理：`BIZ_RESP_SIGNATURE` 与 `BIZ-RESP-SIGNATURE`）。这是有意为之：许多反向代理（如 nginx 的默认配置）会静默丢弃含下划线的 HTTP 头，经过此类代理后只有连字符版本能到达。请任读其一：
...
<Tip>
API 响应使用同一套 Ed25519 签名方案与同一把环境公钥，且携带相同的请求头。因此您可以拿任意一个 API 响应来验证自己的验签器是否能通过 Cobo 的真实签名——无需注册 Endpoint，也无需等待真实推送。
</Tip>

### 读取事件

验证签名之后，请使用 SDK 生成的模型反序列化事件，而不是从解析后的 JSON 中取值：
...
事件载荷是一个判别联合——`data_type` 决定数据模型，交易的 `destination_type` 决定其目标模型——因此像 `destination["address"]` 这样的固定路径只对部分交易成立。模型会自动判定变体，并返回带有该变体字段的对象。完整的处理示例见本页「代码示例」一节。
