...
`fee_station.transaction.*` 事件传递的 `Transaction` 负载 schema 与 `wallets.transaction.*` 事件相同。Webhook 代码对这两类事件使用同一个交易详情 serializer；Fee Station 分支只选择 `fee_station.` 事件类型前缀，并从 `wallet_scopes_info` 中移除钱包范围元数据字段（`custody/waas2/webhooks/managers/wallet_event_by_source.py:34-113`，`custody/waas2/webhooks/managers/wallet_event.py:42-44`）。

`type` 字段不会区分 Fee Station 交易。内部 Fee Station 查询类型会在负载返回前映射为公开的 `Deposit` 或 `Withdrawal` 值（`custody/waas2/transactions/mongo/manager/read/waas_transaction.py:712-727`，`custody/waas2/transactions/dev/enums/transaction.py:104-143`）。
...
<Note>请使用 `cobo_category` 识别 Fee Station 专属交易类别。Fee Station 值由 `TransactionCoboCategory` 定义（`custody/waas2/transactions/mongo/enums/transaction_fields.py:45-63`），并已在 [Cobo-defined categories](/v2_cn/guides/transactions/manage-transactions#cobo-defined-categories) 中说明。</Note>
