...
| ------------- | -------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| Callback     | 类似于 2FA 验证。可选，且按 API Key 配置：当发起提币的 API Key 绑定了 Callback Endpoint 时，该提币需经您的端点批准才能完成；若该 API Key 未绑定 Callback Endpoint，提币将不经此检查。 | 当您使用 WaaS API 发起提币或合约调用时触发。 | 检查交易是否符合预期，并以成功状态码（`200` 或 `201`）和响应体 `ok` 或 `deny` 响应，以指示交易批准或拒绝。 |
...
要创建Endpoint并实现处理逻辑——包括签名验证算法（对 `raw_body|timestamp` 做双重 SHA-256 后以 Ed25519 验证）——请参阅 [设置Callback或 Webhook Endpoint](/v2_cn/guides/webhooks-callbacks/set-up-endpoint)。
