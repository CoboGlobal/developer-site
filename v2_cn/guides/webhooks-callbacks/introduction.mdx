...
每个 Webhook Endpoint 在组织中配置一次，其中包含您在注册时选择的事件类型，还可以选择钱包范围以限制该 Endpoint 接收哪些钱包的事件。请在 Cobo Portal 上注册或更新此范围。请参阅[注册 Webhook Endpoint](https://manuals.cobo.com/cn/portal/developer-console/webhooks-create)。

重试可能会重复投递同一事件，并且 `wallets.transaction.updated` 与终态事件（`succeeded` 或 `failed`）可能描述同一笔交易，因此请将 Webhook 事件视为通知，而不是权威数据源。请使用事件的 `event_id` 对投递进行去重。要核对交易当前的权威状态，请使用 `transaction_id` 调用 [Get transaction information](/v2/api-references/transactions/get-transaction-information) 或 [List all transactions](/v2/api-references/transactions/list-all-transactions)；如果您持有的是 `cobo_id`、`request_id` 或 `transaction_hash`，也可以使用相应标识，而不要仅信任最后收到的 Webhook 负载。

有关不同场景的处理指南，请参阅 [Transaction 事件订阅](/v2_cn/guides/webhooks-callbacks/transaction-event-subscriptions)。
