...
<Warning>
节点连错环境的中继时不会报任何错误——它只是永远不会在另一个环境创建的 vault 中上线，针对该 vault 的密钥生成会无限等待。如果您的 API 调用指向 `https://api.cobo.com`，请在启动节点前设置 `env: production`。
</Warning>
...
### 非交互式初始化（自动化、CI、AI agent）

上文的密码输入需要交互式终端：通过管道向 `init` 喂入密码会直接报错（`cannot attach stdin to a TTY-enabled container`），向 `start` 喂入则会静默挂起。无人值守环境请改用密码文件——TSS Node 程序在每个命令上都支持全局 `--key-file` 参数：
...
<Warning>
密码文件与 `db/secrets.db` 合起来就是私钥分片：请同时备份两者，丢失任何一个即丢失分片。不要把密码本身作为命令行参数传入——机器上的其他用户可以读取进程参数。
</Warning>
...
您现在可以创建包含此 TSS Node ID 的私钥分片持有者组——通过 [Create key share holder group](/v2/api-references/wallets--mpc-wallets/create-key-share-holder-group) 操作，或[在 Cobo Portal 上创建](https://manuals.cobo.com/en/portal/mpc-wallets/ocw/holder-group-overview)。没有单独的注册步骤：持有者组创建成功时，节点即自动完成注册。

<Note>
在持有者组存在之前，中继会以 "node ... is not bound to any app" 拒绝节点连接，节点会持续重试。这是该步骤之前的预期状态，不是故障。
</Note>
