...
有关每种请求类型的完整 Payload 结构，以及您的回调服务器必须返回的 HTTP 响应，请参阅[回调请求和响应格式](/v2_cn/guides/mpc-wallets/server-co-signer/callback-server-data-structure)。

<Note>
这些回调 `request_type` 值由 TSS Node 发送到您的回调服务器以请求批准。它们与 Cobo Webhook 事件类型（例如 `wallets.transaction.*` 和 `wallets.mpc.tss_request.*`）不同，后者由 WaaS 服务发送，用于通知您的 App 状态变更。有关 Webhook 事件类型，请参阅 [Webhook 事件类型](/v2_cn/guides/webhooks-callbacks/webhook-event-type)。
</Note>
