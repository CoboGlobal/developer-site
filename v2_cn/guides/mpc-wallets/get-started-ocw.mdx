...
<img src="/v2_cn/images/guides/MPC_Technical Architecture.svg" className="diagram" alt="完整的 MPC Vault 中的组件"></img>
...
- 私钥分片持有者：每个 Vault 可以有多种私钥分片持有者，包括主控私钥、签名私钥和恢复私钥。每种私钥分片持有者都包括一组 TSS Node，存储可以共同授权交易或恢复根扩展私钥的私钥分片。对于主控私钥和签名私钥，其中一个私钥分片会在 Cobo 多云基础设施中生成并加密存储。要了解更多关于私钥分片持有者组的信息，请参阅[创建持有者组概览](https://manuals.cobo.com/cn/portal/mpc-wallets/ocw/holder-group-overview)。
...
设置完 Vault，创建私钥分片持有者，并生成私钥分片后，您就可以开始创建钱包了。调用[创建钱包](/v2/api-references/wallets/create-wallet)操作，并在请求包体中指定以下属性：
...
- 对于服务端签名人，您可以通过 Cobo Portal 查看签名人是否在线：
    ...
    4. 在**私钥分片管理**页面，找到您的服务端签名人，并查看旁边的状态指示：
        - 🟢 绿点：签名人在线并准备签署交易
        - 无绿点：签名人离线，无法签署交易。

        <img src="/v2_cn/images/guides/greendot.png" className="screenshot_modal" alt="密钥份额管理页面显示签名人状态"/>
