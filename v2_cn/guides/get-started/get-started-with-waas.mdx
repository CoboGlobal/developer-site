...
在发送 API 请求时，您需要在请求头中包含 API Key、`Biz-Api-Nonce`（当前 Unix 时间戳（毫秒））和 API 签名，以作为身份认证信息。有关更多信息，请参阅 [Cobo Auth](/v2_cn/guides/overview/cobo-auth)。
...
- Biz-Api-Nonce：当前 Unix 时间戳（毫秒），用作请求的 nonce。
