...
在充币之前，请确认相关链已为您的团队启用，且代币已上线并开放充币。仅当代币的 `can_deposit` 属性为 `true` 时，该代币才支持充币。请在转入资金前确认此属性，因为向尚未启用的链或代币充币可能无法被自动检测或入账。

如需检查充币资格，请在充币前查询代币配置。如果您已向当时未启用的代币充币，请勿继续向同一地址转入资金；请改用 [List all transactions](/v2/api-references/transactions/list-all-transactions) 确认该笔转账的当前状态。

## Memo 和 Tag 要求

部分链除了充币地址外还需要提供 Memo 或 Tag，以便将充币正确路由到对应账户。当代币需要 Memo 或 Tag 时，其 `require_memo` 属性为 `true`。请在充币前务必检查此属性，并在每次向该代币充币时附上准确的 Memo 或 Tag。

如果充币时未附带所需的 Memo 或 Tag，或填写了错误的 Memo 或 Tag，该笔充币可能无法自动入账，需要人工处理后资金才会到账。如需确认此类充币是否已被接收，请使用 [List all transactions](/v2/api-references/transactions/list-all-transactions)。
...
部分链仅支持特定的交易格式或版本进行充币。以不受支持的交易格式提交的充币可能无法被检测或入账。在接入新链时，请先确认您的充币使用受支持的交易格式，再大规模转入资金。
