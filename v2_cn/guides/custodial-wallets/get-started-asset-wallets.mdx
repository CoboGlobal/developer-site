...
部分链除了充币地址外还需要提供 Memo 或 Tag，以便将充币正确路由到对应账户，例如 XRP、XLM 和 EOS。向这些链充币时如果未附带所需的 Memo 或 Tag，或填写了错误的 Memo 或 Tag，该笔充币可能无法自动入账。
...
在代币上线并开放充币之前发起的充币，可能在转账时无法被检测或入账。充币前请按 [充币前提条件](#充币前提条件) 所述确认代币已启用。如需检查此类充币之后是否已被记录，请使用 [List all transactions](/v2/api-references/transactions/list-all-transactions)。
