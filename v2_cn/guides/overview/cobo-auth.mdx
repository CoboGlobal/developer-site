...
您需要在请求头中提供 API Key、`Biz-Api-Nonce`（当前 Unix 时间戳（毫秒））和 API 签名：
...
- Biz-Api-Nonce：当前 Unix 时间戳（毫秒）。该值用作请求的 nonce。
...
在 WaaS 2.0 身份验证中，Nonce 指请求头 `Biz-Api-Nonce` 的值。该值必须是当前 Unix 时间戳（毫秒）。
...
1. 首先，基于您的请求拼接出一个字符串，如下所示：
    ...
    | NONCE     | 当前 Unix 时间戳（毫秒）。该值必须与请求头 `Biz-Api-Nonce` 的值完全相同。 | `1718587017026`                                                |
